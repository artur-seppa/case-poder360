...
A resposta discursiva da Parte 2 (arquitetura para notificação em tempo real de 500 mil+ usuários simultâneos) está em [`docs/parte-2-arquitetura.md`](docs/parte-2-arquitetura.md).
...
- **`app/Actions`**: um caso de uso por classe (`CreateTask`, `LoginUser`, `RefreshAccessToken`...), reaproveitado pelo Controller web e pelo Controller da API.
...
Único jeito suportado de rodar o projeto, pois tem a integração com outras tecnologias: o `.env.example` já vem configurado
...
- **Factories** (`database/factories/{User,Task}Factory.php`): todo teste monta seus próprios dados via `User::factory()->create()`/`Task::factory()->for($user)->create()` em vez de fixtures fixas ou seed do banco. Cada teste fica isolado e explícito sobre o que precisa (ex.: `Task::factory()->for($owner)->create()` vs. `Task::factory()->create()` pra simular a tarefa de "outro usuário"), sem depender de estado deixado por outro teste; o `RefreshDatabase` (`tests/Pest.php`) garante um banco limpo a cada teste.

## Autenticação

- **Web**: sessão via cookie (Breeze), rotas padrão de registro/login/logout/reset (ver tabela em [Rotas](#rotas)). `login` usa o lockout nativo do Laravel (5 tentativas por e-mail+IP, com backoff); as demais têm `throttle:5,1` de rota. O cookie "lembrar-me" (`remember_token`) tem validade reduzida de ~400 dias (padrão do Laravel) pra `REFRESH_TOKEN_TTL_DAYS` (7 dias, mesma janela do refresh token da API). Isso é configurado em `AppServiceProvider::boot()`, já que, ao contrário do refresh token da API, esse cookie não rotaciona a cada uso.
- **API**: rotas em `/api/v1/auth/*` e `/api/v1/user/*` (ver tabela em [Rotas](#rotas)). Login/registro retornam `access_token` no corpo da resposta (expira em 15 min) e definem um cookie `refresh_token` httpOnly (expira em 7 dias por padrão, configurável via `REFRESH_TOKEN_TTL_DAYS`, rotacionado a cada uso em `/auth/refresh`). O cookie tem `path=/api/v1/auth`, então só é enviado nessas rotas; o endpoint `refresh` autentica via esse cookie, não via header. `reset-password` (com o token do e-mail) e `PUT /user/password` (autenticado, com a senha atual) atendem fluxos diferentes: o primeiro é pra quem esqueceu a senha e não consegue logar, o segundo pra quem já está logado e quer trocar. Qualquer um dos dois, ou deletar a conta, revoga **todos** os tokens do usuário, incluindo o usado na própria requisição, exigindo login de novo depois.
...
Registro/login/logout/reset de senha ficam nas rotas padrão do Breeze:

| Método | Rota | Descrição | Rate limit |
|---|---|---|---|
| GET | `/register` | Formulário de registro | — |
| POST | `/register` | Cria a conta | 5/min |
| GET | `/login` | Formulário de login | — |
| POST | `/login` | Autentica | 5/e-mail+IP (lockout) |
| POST | `/logout` | Encerra a sessão (autenticado) | — |
| GET | `/forgot-password` | Formulário de "esqueci minha senha" | — |
| POST | `/forgot-password` | Envia e-mail com link de reset | 5/min |
| GET | `/reset-password/{token}` | Formulário de nova senha | — |
| POST | `/reset-password` | Define a nova senha | 5/min |

Mais detalhes de cada fluxo na seção [Autenticação](#autenticação).
...
A documentação interativa da API (gerada com [Scribe](https://scribe.knuckles.wtf/), tema [Scalar](https://github.com/scalar/scalar)) fica em `http://localhost:8000/docs`. Depois de mudar uma rota, request ou resource da API, regenere com:
...
- Rate limiting: ver [Funcionalidades adicionadas](#funcionalidades-adicionadas).
...
- **Mailpit**: captura os e-mails enviados em dev/Docker sem entregar de verdade, com uma UI (`localhost:8025`) pra inspecionar o conteúdo, usada pra testar visualmente o fluxo de reset de senha.
- **Paginação (offset, não cursor)**: a listagem de tarefas (`tasks.index` e `GET /api/v1/tasks`) usa `paginate()` (offset/limit), não `cursorPaginate()`, por dois motivos concretos, além da escala pequena por usuário. Primeiro, a ordenação atual é `->latest()` (por `created_at`), sem coluna de desempate única: inofensivo pra offset, que corta por posição, mas arriscado pra cursor, que decide a fronteira de cada página comparando o valor da última linha vista. Se duas tarefas caírem no mesmo `created_at` (bem possível aqui, já que o seeder cria 20 tarefas em sequência rápida pro usuário demo), o cursor pode pular ou duplicar uma linha na borda da página; resolver isso exigiria uma coluna de desempate única (ex. `id`) na ordenação. Segundo, cursor pagination não pula direto pra um número de página arbitrário (não existe "página 5", só navegação sequencial via cursor), o que combina mal com o paginador numerado padrão de uma listagem simples de CRUD. O trade-off que abrimos mão: o custo de `OFFSET` cresce com a página, porque o banco escaneia e descarta todas as linhas anteriores antes de retornar a página pedida (cursor salta direto via índice), e a paginação pode "derivar" se linhas forem inseridas ou removidas entre uma página e outra. Nenhum dos dois pesa aqui, já que a lista é pequena e escopada por usuário, sem caso real de deep pagination. O usuário demo (`demo@example.com`) é semeado com 20 tarefas (> 15/página) para a paginação ficar visível (2 páginas) ao rodar o seeder.
